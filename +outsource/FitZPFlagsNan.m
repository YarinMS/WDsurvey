function [Obj] = FitZPFlagsNan(Obj,Args)
    ...
    arguments
        Obj MatchedSources
        Args.MinNumSrc         = 20;
        Args.MagField char     = 'MAG_APER_3';
        Args.MagErrField char  = 'MAGERR_APER_3';
        Args.HardFlagsMask     = FlagIdentification(Obj);
    end
    ...
    %% Throw out bad data
    ApplyMask(Obj, 'Mask',Args.HardFlagsMask,'Fields',{Args.MagField,Args.MagErrField}, 'Value', NaN);
    ToMuchNaNMask=sum(isnan(Obj.Data.(Args.MagField)),1)/Obj.Nepoch > 0.75;%mask for source withmore than 0.75% NaN
    ApplyMask(Obj, 'Mask',~ToMuchNaNMask, 'Value', 'erash');
