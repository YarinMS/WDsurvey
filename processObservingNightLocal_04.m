function processObservingNightLocal(mount, telescope, year, month, day, batchSize)
    ...
    for b = 1:length(batches)
        ...
        for cropId = uniqueCropIdsFits(10)
            args.CropID = cropId;
...
function [FP,results,lcData] = applyFP(AI,wdTable,Iwd)
    ...
    dec = wdTable.Dec(Iwd);
     ...
     FP = imProc.sources.forcedPhot(AI, 'Coo', [ra, dec],'ColNames', {'RA', 'Dec', 'X', 'Y', 'Xstart', 'Ystart', 'Chi2dof', 'FLUX_PSF', 'FLUXERR_PSF', 'MAG_PSF', 'MAGERR_PSF', 'BACK_ANNULUS','STD_ANNULUS', 'FLUX_APER', 'FLAG_POS', 'FLAGS'},'MomentMaxIter',50,'UseMomCoo',true,'HeaderZP',true,'ReconstructPSF',true,'constructPSFArgs' , {'RepopulatePSF' true 'ThresholdPSF' 20 'RangeSN' [50,1000] 'RadiusPSF' 6} );
    ...
    try
        ...
        limMag =19.5*ones(length(airmass),1);
    end
