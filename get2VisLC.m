...
for Icol = 1 : numel(Results.SubFrame{wdIdx})
    ...
    if ~isempty(Found_Index)
        ...
        for Ifind = 1: length(Found_Index)
              ...
              [maso,event,Npts,RMS0,RMS1] = getLCsysC4( Results.Main{Ibatch,MainCol},'SourceIdx',Found_Index(Ifind),'SaveTo',WD.Data.save_to,'Serial',...
                  ...
                  'SDcluster',true,...
                  'Title',sprintf('%i.%i.%i.%i-%s-%s-%.3f ',Ifind,wdIdx,Ibatch,Results.SFcol(MainCol),VN{Ibatch,MainCol},DataStamp(6:end),WD.G_Bp(wdIdx)),'SaveLC',true);
