...
if ~isempty(Args.FileNames )
        ...
        for Ivisit = 1 : height(Args.FileNames.FileName)
        
        cd(Args.FileNames.Folder{Ivisit})
        fprintf('\nGeting Obs info from proc dir %i / %i \n',Ivisit,height(Args.FileNames.FileName))
...
sfile= strrep(sfile, ' ', '');
    ...
    file_name = [sprintf('Observation \\ night \\ Info \\ %s_ \\ %s',Args.Field,Cats(1).name(1:22)),'.png'];
    sfile = strcat(Args.save_to,file_name);
    sfile= strrep(sfile, ' ', '');
end
