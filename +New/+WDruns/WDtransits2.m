...
function batches = groupBatches(matchedSources, Nvisits)
    ...
    Tab = table(subframes, wdIds, Ras, Decs, Gmag, Bpmag, Pwd, visitsFound, totalVisits, overlapFlags, badFlags, contaminated, avgDetections, ...
        ...
        function plotDetectionResults(results, Iwd, Ibatch, args, plotFlag, outputDir)
            ...
            % Extract x and y fields and save them as a text file
        if isfield(results{Iwd,Ibatch}, 'x') && isfield(results{Iwd,Ibatch}, 'y')
            xData = results{Iwd,Ibatch}.x;
            yData = results{Iwd,Ibatch}.y;

        % Combine x and y into a two-column matrix
            data = [xData(:), yData(:)];

        % Define the filename and save the matrix as a text file
            filename = fullfile(outputDir, sprintf('results_WD%d_Batch%d.txt', Iwd, Ibatch));
