%% 
function [MetaTable] = SignalHunter2(MetaTable,mount, telescope, year, month, day, batchSize,args)
 ...
 arguments
   ...
   args.Date = sprintf('%i - %i - %i',year,month,day);
    ...
    for Ifield = 1: Nfields
        fprintf('%s %i/%i fields\n',uniqueFields{Ifield},Ifield,Nfields)
       ...
       % Intilize WD table
        ...
        for b = 1 : length(Batches)
            fprintf('%s : Batch # %i / %i \n',uniqueFields{Ifield},b,length(Batches))
            ...
            for cropID = 1:24
                ...
                if ~isempty(List.FileName)
                    ...
                    ms = mergeByCoo(MS, MS(randi(length(MS))));
                    ms.bestMag;
                    MMS = calibMS(ms);
                    MMS.FileName = {MS.FileName};
                    MMS.UserData.CropID = cropID;
    ...
    save(sprintf('/media/yarinms/Data2/Projects/MarvinRuns/%s/Temp_Results_Table_%s_%s_%04d.%02d.%02d.mat',args.ID,args.Date,args.Tel,year,month,day),'MetaTable','-v7.3')
end