function [Cand, WDcand, FlagComb, ReportFile] = findVariableCandidates(ms, Args)
    ...
    if any(FlagComb)
        ...
        if Args.Report
            [Rpt, ch1] = initializeReport(Args.args.reportFN);
        end
        ...
        % Close the report if reporting was requested
        if Args.Report
            ReportFile = finalizeReport(Rpt, ch1);
        end
    end
end
...
function [Rpt, ch1] = initializeReport(TitlePage1)
    % Initialize a PDF report
    import mlreportgen.report.*;
    import mlreportgen.dom.*;
    reportFileName = fullfile('~/Documents/Temp/', TitlePage1);
...
function ReportFile = finalizeReport(Rpt, ch1)
    append(Rpt, ch1);
    close(Rpt);
    ReportFile = Rpt.OutputPath;
end
