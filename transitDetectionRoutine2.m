function transitDetectionRoutine2(visitGroup, Args)
    arguments
        visitGroup
        Args.def = 0;
    ...
    visitPath = visitGroup{1};

    % Define regex pattern and parse tokens
    pattern = '/(?<telescopeID>LAST\.\d{2}\.\d{2}\.\d{2})/(?<year>\d{4})/(?<month>\d{2})/(?<day>\d{2})/proc/(?<visitID>\d{6}v0)';
    tokens = regexp(visitPath, pattern, 'names');
    year = tokens.year; month = tokens.month; day = tokens.day;
    telescopeID = tokens.telescopeID; visitID = tokens.visitID;

    % Set up report paths
    catalogReportFile = fullfile(Args.saveDir, sprintf('%s_Catalog_Report_%s_%s_%s.pdf', telescopeID, year, month, day));
    rptCatalog = initializeReport(catalogReportFile, 'Catalog Analysis Report');
    fpReportFile = fullfile(Args.saveDir, sprintf('%s_FP_Report_%s_%s_%s.pdf', telescopeID, year, month, day));
    rptPhotometry = initializeReport(fpReportFile, 'Forced Photometry Report');

    fprintf('Processing transit detection on %d visits... (%s)\n', numel(visitGroup), visitID);

    % Extract data files
    [fitsFilesBatch, hdf5FilesBatch, fitsFilesNames, hdf5FilesNames, fitsFilesFolders, hdf5FilesFolders] = extractDataFiles1(visitGroup);
    ...
    for c = 1:length(uniqueCropIds)
        ...
        msAll = processCatalogData(cropId, hdf5FilesNames(strcmp(cropIdsHdf5, cropId)), hdf5FilesFolders, Args);
        if ~isempty(msAll)
            [Cand, WDcand, ~, resultChapter] = findVariableCandidates4(msAll, 'args', Args);
            append(rptCatalog, resultChapter);
        end

        % White Dwarf Detection and Processing
        [RA, Dec, fieldCoords, ~] = getMScoords(subframeFitsFiles{1});
        wdSources = findWhiteDwarfs(RA, Dec, fieldCoords);

        % Appending WD analysis if applicable
        if ~isempty(wdSources)
            forcedChapter = createChapter(sprintf('Batch %d, Subframe (CropID): %s', c, cropId));
            appendWDSummaryToReport(forcedChapter, visitID, cropId, wdSources, Args, FPAI);
            append(rptPhotometry, forcedChapter);
        end

        waitbar(c / length(uniqueCropIds), h, sprintf('FieldID %s\nVisitID %s\nCropID #%d\n', Args.FieldID{1}, visitID, c));
    end
...
function [fitsFilesBatch, hdf5FilesBatch, fitsFilesNames, hdf5FilesNames, fitsFilesFolders, hdf5FilesFolders] = extractDataFiles1(batch)
...
function [FP,results,lcData] = applyFP(AI,wdTable,Iwd,momentMaxIter)
    ...
    try 
        ...
        lcData.Ctrl = lcData.lc;
      
        enssembeleLC = nan*ones(size(lcData.lc));
        deltaMag = lcData.lc - enssembeleLC;
        relFlux = 10.^(-0.4 * deltaMag);
        lcData.relFlux = relFlux / median(relFlux, 'omitnan');
        lcData.typicalSD = std(lcData.lc, 'omitnan');
...
function [mms] = cleanBadSources1(ms,args)
...
function appendWDSummaryToReport(forcedChapter, b, cropId, wdSources, args, FPAI)
    ...
    batchInfoPara = Paragraph(sprintf('visitID: %s, Subframe (CropID): %s Nwd: %d', b, cropId,height(wdSources)));
    ...
    try
        % Plot FWHM vs JD
        subplot(3, 1, 2);
        plot(args.catJD, args.FWHM, '-.', 'Color', [0.8500 0.3250 0.0980],'LineWidth',2);
        xlabel('JD');
        ylabel('FWHM');
        title('FWHM vs JD');
        grid on;
        ...
        % Plot Airmass vs JD
        subplot(3, 1, 1);
        plot(args.catJD, args.airmass, '-.', 'Color', [0.2941, 0.0000, 0.5098],'LineWidth',2);
        xlabel('JD');
        ylabel('Airmass');
        title('Airmass vs JD');
        ...
        subplot(3, 1, 3);
        plot(args.catJD, args.LimMag, '-.', 'Color', [0.5451, 0.0000, 0.000],'LineWidth',2);
        xlabel('JD');
