...
%% target
%	1.8005047
y = [22006.43	  ; 22103.733	 ; 22360.45	 ; 22412.898 ; 22403.46	
     22573.59 ; 22237.806	;22232.246		 ; 22534.739 ; 	22362.381	;
     23125.316	; 23029.789	 ; 22435.174	; 22923.159	 ;22268.409	 ; 
     21990.505	; 22172.546	; 22010.557 ; 22494.571;22460.86	
]

bkg = 113*[186.654  ; 186.873 ;  184.939 ;186.041; 187.788 ;
       187.262  ; 187.24 ; 186.922 ; 187.223 ; 192.215;
       196.963 ; 191.216 ; 186.461 ; 189.661; 188.504;
       187.516; 186.85 ; 187.252 ; 188.353 ; 187.379
]

s = y - bkg;
Snr = s./sqrt(s + (bkg+113*25))
...
263.6530551649709	60.97255259195575	18.038418
...
plot([1:20],msStar.Data.BACK_ANNULUS(:,index_psf),...
 ...
 Lg{7} = sprintf('Manual Background  ; Scatter : %.3f \\%%',100*RobustSD(bkg./113)/mean(bkg./113))
...
I = [];
RMScat = [];
for i = 1: length(msStar.Data.FLUX_APER_3(1,:))
    if sum(~isnan(msStar.Data.FLUX_APER_3(:,i))) == 20
        
        coord = [coord ; mean(msStar.Data.RA(:,i)) mean(msStar.Data.Dec(:,i)) ];
        I = [I;i];
        RMScat = [RMScat ; std(msStar.Data.FLUX_APER_3(:,i))/ mean(msStar.Data.FLUX_APER_3(:,i))];
    else
...
for k = 440 :length(I)
                 ...
                 tic;
                  FP_all = imProc.sources.forcedPhot(AI,'Coo',[coord(k,:)],'ColNames',{'RA','Dec','X','Y','Xstart','Ystart','Chi2dof','FLUX_PSF','FLUXERR_PSF','MAG_PSF','MAGERR_PSF','BACK_ANNULUS', 'STD_ANNULUS','FLUX_APER','FLAG_POS','FLAGS'});
                  to     = toc;

                  fprintf('\nFinished analyazing source %.0f out of %.0f', I(k),length(I))
                  fprintf('\nFor subframe # %d',Star1.CropID)
                  fprintf([' \n only "',num2str(to) ,'" s'])
                  
                  fpall  = FP_all.Data.FLUX_APER_3(:,1);
                  RMSforced = [RMSforced ; std(fpall)/mean(fpall)];
...
for i = 1: length(I)
         ...
         for Iimg =  1 :20
               ...
               [RA, Dec] = AC(Iimg).getLonLat('deg');
               [~,inx_cat] = min(abs(RA - Star1.RA))
               [~,inx_cat1] = min(sqrt((Dec - coord(i,2)).^2 + (RA -coord(i,1)).^2));
         ...
         if sum(~isnan(LC)) == 20
              ...
              %coord2 = [coord ; mean(msStar.Data.RA(:,i)) mean(msStar.Data.Dec(:,i)) ];
      
              RMSpsfcat = [RMScat ; std(LC) / mean(LC)];
         else
...
for k = 1:length(I)
                 ...
                 tic;
                  ...
                  fprintf('\nFinished analyazing source %.0f out of %.0f', k,length(I))
                  fprintf('\nFor subframe # %d',Star1.CropID)
                  fprintf([' \n only "',num2str(to) ,'" s'])
