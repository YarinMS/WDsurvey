...
for Iobj = 1 : Nobj
    ...
    if wd.CropID(Iobj) > 0 
      ...
      % tit.Interpreter = 'latex' ; 
       ...
       lg = legend([...
           ' RobustSD $\approx$ ',num2str(SDrobust1),' ; $5-\sigma$ detections : ',num2str(100*length(m1)/length(FP.JD)) ],['Lim Mag for SF # ',num2str((ID(1)))])%,['SF = ', num2str(ID(2)),' ; Dist from edge $\approx$ '...
