function [lcData,res] = getCatLC3(mms,wdTable,args)
 ...
 args.Nvisits = 1000;
                    ...
                    [~,fname,~] = fileparts(wdTable.Date);
                    part = strsplit(fname,'.');
                    lcData{Ibatch}.Tel = wdTable.Telescope;
                    lcData{Ibatch}.Date = part{:};
                    %lcData{Ibatch}.Table.FWHM = FWHM;
                    %lcData{Ibatch}.Table.Airmass = AM;
                    %lcData{Ibatch}.Table.visTable = Table;
                    lcData{Ibatch}.Table.FieldID  = wdTable.FieldID;
                    ...
                    lcData{Ibatch}.Table.Pwd = wdTable.P_wd;
                    lcData{Ibatch}.Table.AbsMag= wdTable.Abs_Mag;
                    %lcData{Ibatch}.Table.Total_Visits = Table.Nvisits;
                    %lcData{Ibatch}.Table.Visits_Found= length(batch) ;
                    lcData{Ibatch}.Table.Subframe = wdTable.CropID;
...
%                    lcData{Iwd,Ibatch} = transitSearch.handleNaNValues(lcData, numel(catJD));
                  ...
                  if isfield(lcData{Ibatch},'Res')
                      ...
                      % in the future
                        ...
                        %Nnans =  sum(nanIdx);
                        lcData{Ibatch}.nanIndices = isnan(lcData{Ibatch}.lc);
                        args.Ndet = 20;
                        ...
                        [AbsMag, Plx, Dist, NonSingleStar, Neighbors, Identifiers] = getWDParams2(wdTable.RA, wdTable.Dec);
