function Data = SSHunpackFitsFilesMarv(Data)
    % SSH into the remote system and get the list of files
    eventInfo  = Data;
    timeWindow = [Data.minTime Data.maxTime]; 
    Data.timeWindow = timeWindow;
    ipAddress = calculateIpAddress(eventInfo);
    Data.ipAddress = ipAddress;

    % Get remote path from filename
    FF = Data.FileNames{1}{1};
    spltPth = strsplit(FF, '/');
    % Compute computerID
    computerID = sprintf('last%02d%s', str2double(eventInfo.telescope(9:10)), determineEastWest(eventInfo));

    % Construct remotePath
    remotePath = fullfile('/', computerID, determineDataDir(eventInfo), 'archive', ...
    spltPth{5}, spltPth{6}, spltPth{7}, spltPth{8}, 'raw');
    ...
    listFilesCommand = sprintf( ...
        'sshpass -p "physics" ssh ocs@%s "cd %s && ls *sci_raw*.fits.fz"', ...
    ...
    startTime = datetime(timeWindow(1), 'convertFrom', 'jd');
    endTime = datetime(timeWindow(2), 'convertFrom', 'jd');
   
    
    % endTime   = datetime(timeWindow(2), 'InputFormat', 'yyyyMMdd.HHmmss.SSS');
    ...
    if isempty(filesToDecompress)
        ...
        listPrevDayFilesCommand = sprintf( ...
            'sshpass -p "physics" ssh ocs@%s "cd %s && ls *.fits.fz"', ...
    ...
    decompressCommand = sprintf( ...
        'sshpass -p "physics" ssh ocs@%s "cd %s && ', ...
        ipAddress, remotePath);
...
function ensureRemoteScriptExists(ipAddress, localScriptPath, remoteScriptPath)
    ...
    checkScriptCommand = sprintf( ...
        'sshpass -p "physics" ssh ocs@%s "test -f %s"', ...
    ...
    if status ~= 0
        fprintf('runRemotePipelineScript.m not found on remote machine. Uploading it...\n');
        scpCommand = sprintf( ...
            'sshpass -p "physics" scp %s ocs@%s:%s', ...
...
function runRemotePipeline3(ipAddress, telescopeNumber, year, month, day)
    ...
    localScriptPath = '~/Documents/WDsurvey/+marvinFP/runRemotePipelineScript.m'; % Local path of the script
    remoteScriptPath = '/home/ocs/Documents/MATLAB/runRemotePipelineScript.m'; % Remote path


    ensureRemoteScriptExists(ipAddress, localScriptPath, remoteScriptPath) 
    % Build the MATLAB command to run the pipeline remotely
    matlabCommand = sprintf([ ...
        'runRemotePipelineScript(%d, %d, %d, %d);', ...  % Pass the parameters to the script
        'exit;'], telescopeNumber, year, month, day);

    % SSH command to execute MATLAB remotely
    remoteMatlabCommand = sprintf( ...
        'sshpass -p "physics" ssh ocs@%s "matlab -nodesktop -nodisplay -r ''%s''"', ...
        ipAddress, matlabCommand);
