function [stackedWDtable,mainMS,mainObsData] = getMSDateTgt(mount, telescope, year, month, day, batchSize,args)
...
% Create path
 arguments
   ...
   if args.FieldID ~= "" % Check if the field ID is provided as an argument
        % Validate the provided field ID
        fieldMatch = strcmp(uniqueFields, args.FieldID);
        if any(fieldMatch)
            selectedFieldIdx = find(fieldMatch, 1);
            selectedField = uniqueFields{selectedFieldIdx};
            fprintf('Using provided field ID: %s\n', selectedField);
        else
            error('The provided FieldID "%s" is not valid.', args.FieldID);
        end
    else

        % Display unique fields to the user
        fprintf('Available Fields:\n');
        for i = 1:Nfields
            fprintf('%d: %s\n', i, uniqueFields{i});
        end
    
        % Ask the user to select a field
        fprintf('\nPlease select a field by its number:\n');
        selectedFieldIdx = input('Enter the field number: ');
    
        % Validate the input
        while selectedFieldIdx < 1 || selectedFieldIdx > Nfields
   ...
   if ~isempty(args.TgtCoord)
       wd = isWD([],args.TgtCoord(1),args.TgtCoord(2));
       if ~isempty(wd.Table)
        wd = wd.Table;
        wd.RA = wd.RA*180/pi;
    ...
    for Ifield = selectedFieldIdx % 1: Nfields
       ...
       % Intilize WD table
        ...
        for subframeIdx = args.CropID
            ...
            wdSources = findWhiteDwarfsM(subframeRA, subframeDec, fieldCoordsSF);
        ...
        for b = 1 : length(Batches)
            ...
            for cropID = args.CropID
                ...
                if args.getMS
                    mainMS{end+1} = MS;
                    mainObsData{end+1} = obsData;
                end

                if ~isempty(args.TgtCoord)
                    Ind = MS.coneSearch(wd.RA,wd.Dec,6)

                    isNonEmpty = any(arrayfun(@(x) ~isempty(x.Ind), Ind))

                    if isNonEmpty
                        fprintf('\nAt least one Ind.Ind is not empty. CropID %i \n', cropID);
                    else
                        disp('All Ind.Ind fields are empty.');
                    end
                    ...
                    if isempty(mms) %|| sum(nanIdx) > 0.30*length(nanIdx)
                        ...
                        if isfield(lcData,'Res') || sum(nanIdx) > 0.3*length(nanIdx)
                                ...
                                if batchData.ValidPoints > 0.85*length(lcData.lc)
                                    WD.BatchDetections = WD.BatchDetections +1;
                                    WD.Detected = true;
                                end
