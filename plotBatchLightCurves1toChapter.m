function plotBatchLightCurves1toChapter(WD, saveDir, args)
    ...
    for batchIdx = 1:numBatches
        ...
        if catFlag || forcedFlag
            ...
            if isfield(catalogBatch,'RMS')
                    catRMS = catalogBatch.RMS;
                else
                    catRMS = [];
                end

                if isfield(forcedBatch,'RMS')
                    forcedRMS  = forcedBatch.RMS;
                else
                    forcedRMS = [];
                end


            if catFlag && forcedFlag && isfield(catalogBatch,'Event') && isfield(forcedBatch,'Event')
                ...
                lcParagraph = Paragraph();
                lcParagraph.Style = {FontSize('10pt')}; 
                append(lcParagraph, Text('Detection in both Catalog and Forced Data:'));
                append(args.ReportChapter,lcParagraph)

                plotAndSaveLightCurves2Chapter(forcedBatch.Results, forcedBatch.lcData, ...
                    catalogBatch.Results, {catalogBatch.lcData}, saveDir, WD, 1,args.ReportChapter,'catRMS',catRMS,'forcedRMS',forcedRMS);
            elseif isfield(catalogBatch,'Event') && ~isfield(forcedBatch,'Event')
                % Catalog only (Strange)
                catalogBatch.Results.res.Methods = catalogBatch.Methods;
                catalogBatch.Results.res.FluxMethods = catalogBatch.FluxMethods;
                catalogBatch.lcData.Tel = WD.FieldID;
                catalogBatch.lcData.Date = WD.ObsDate;
                lcParagraph = Paragraph();
                lcParagraph.Style = {FontSize('10pt')}; 
                append(lcParagraph, Text('Detection in Catalog (check FP data):'));
                append(args.ReportChapter,lcParagraph)
                plotAndSaveSingleLightCurve2Chapter(catalogBatch.Results, catalogBatch.lcData, saveDir, WD, 1,args.ReportChapter,'catRMS',catRMS,'forcedRMS',forcedRMS);
            elseif ~isfield(catalogBatch,'Event') && isfield(forcedBatch,'Event')
                % Forced only (Weak?)
                % you need to mention  it is a forced photmetry result
                lcParagraph = Paragraph();
                lcParagraph.Style = {FontSize('10pt')}; 
                append(lcParagraph, Text('Detection in Catalog (check FP data):'));
                append(args.ReportChapter,lcParagraph)
                plotAndSaveSingleLightCurve2Chapter(forcedBatch.Results, forcedBatch.lcData, saveDir, WD, 1,args.ReportChapter,'catRMS',catRMS,'forcedRMS',forcedRMS);
            end
