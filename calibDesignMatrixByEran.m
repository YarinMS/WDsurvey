function [H, ColNames]=calibDesignMatrix(Nimage, Nstar, Args)
    ...
    if Args.Sparse
       ...
       H = sparse([],[],[], Nline, 2*Nimage + Nstar   + NpropImage, Npopulated);
    ...
    for Ist=1:1:Nstar
        ...
        if ~isnan(Args.StarProp(Ist))
       
            
            H(Iline, length(VecIm)+VecIm) = Args.StarProp(Ist).*diag(ones(Nimage,1));
        else
            H(Iline, length(VecIm)+VecIm) = 0.*diag(ones(Nimage,1));

        end
        H(Iline, 2*Nimage+Ist) = ones(Nimage,1);
       % Ip=1:1:NpropStar
        %    H(Iline, Nimage+Nstar + Ip) = Args.StarProp{:}(Ist).*diag(ones(Nimage,1));
