function transitDetectionRoutine(visitGroup,Args)
    % TRANSITDETECTIONROUTINE Process a group of visits for transit detection
    % Input:
    %   visitGroup - Cell array of file paths or directory names for the visits in the group
    ...
    % Extract each component from the tokens structure
    year = tokens.year;
    month = tokens.month;
    day = tokens.day;
    telescopeID = tokens.telescopeID;
    visitID = tokens.visitID;
    
    

    catalogReportFile = fullfile(Args.saveDir, sprintf('%s_Catalog_Report_%s%s%s_%s.pdf',telescopeID, year, month, day,visitID));
    rptCatalog = initializeReport(catalogReportFile, 'Catalog Analysis Report');
    
    % Initialize the forced photometry report
    fpReportFile = fullfile(Args.saveDir, sprintf('%s_FP_Report_%s%s%s_%s.pdf',telescopeID, year, month, day,visitID));
    ...
    % Extract CropIDs from FITS and HDF5 file names
        ...
        for c =1: length(uniqueCropIds)
          ...
          %  FPAI = generateFPImg(cropId,subframeFitsFolders,subframeFitsNames);
            ...
            catalogSection = createChapter(sprintf('VisitID %s, Subframe (CropID): %s', visitID, cropId));
            ...
            if isempty(msAll)
                ...
                WDcand = [];
                 ...
                 [Cand, WDcand, FlagComb,resultChapter] = findVariableCandidates4(msAll, 'args',Args,'catalogChapter', catalogSection);
            ...
            forcedMChapter = createChapter(sprintf('Visit ID %s, Subframe (CropID): %s Nwd: %d', visitID, cropId,height(wdSources)));
