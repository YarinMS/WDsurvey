...
for Iobj = 1 : Nobj
    ...
    if wd.CropID(Iobj) > 0 
      ...
      % FP.Data.MAG_PSF(flags(:,1),:) = NaN;
       ...
       fprintf('\nDID NOT cleaned %d Data points by Flags \n ',sum(1))
      ...
      % lg = legend(['RobustSD $\approx$ ',num2str(),' ; $5-\sigma$ detections : ',num2str(100*length(m)/length(FP.JD)),'% '], ...
       %    ['Lim Mag for SF # ',num2str(ID(1))])%,['SF = ', num2str(ID(2)),' ; Dist from edge $\approx$ '...
       legends = cell(1, 2)
       legends{1} = sprintf('RobustSD = %.2f  ; %.2f % of entrie obs ', [SDrobust1 100*length(m)/legnth(FP.JD)]);
       legends{2} = sprintf('Gaia g mag = %.2f', ID(1));
       legend(legends(1:2),'Location','best','Interpreter','latex')
       %lg.Interpreter = 'latex';
       %lg.Location = 'best' ;
       hold off
