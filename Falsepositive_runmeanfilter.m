   ...
   %         'title',sprintf('$R_p$ = %.3f $R_{\\oplus}$ ; $a$ = %.3f ; $\\Delta Mag$ = %.3f ; $\\delta_{flux} = %.3f$',Radii(radius_ind)/(6387e3),SemiMajjors(Semi_ind),max(LC_injected - mean(LC_injected)),...
    %            1 - min(LCmodel)),'Model',deltamag);
            ...
            for Ifilt=1:1:Nfilt
                ...
                ResFilt = timeSeries.filter.runMeanFilter(LC, Args.runMeanFilterArgs{:}, 'WinSize',Args.runMeanFilterWinSize(Ifilt));
                if Ifilt==1
...
save('W5S5FAP.mat','res5','-v7.3')
save('W4S5FAP.mat','res4','-v7.3')
save('W3S5FAP.mat','res3','-v7.3')
save('W2S5FAP.mat','res2','-v7.3')
