...
%%  CropID coords cross match with WD catalogs     
        function [Results, wd] = findWhiteDwarfsInField(matchedSources,args)
            ...
            for Iid = 1 : 24 
               ...
               obsParams = transitSearch.EfficientReadFromCat(ms1.FileName, Iid); % you check only first visit which might be a problem
                ...
                if iscell(batch) 
                    ...
                    for Ibatch = 1 : length(batch)
                        ...
                        for Ivis = 1 : numel(batch{Ibatch})
                            ...
                            OP        = transitSearch.EfficientReadFromCat(batch{Ibatch}(Ivis).FileName, table.Subframe(Iwd));
                    ...
                    lcData{Iwd,Ibatch} = transitSearch.extractLightCurve(mms, table.RA(Iwd), table.Dec(Iwd),args);
                    lcData{Iwd,Ibatch}.limMag = LimMag;
                    lcData{Iwd,Ibatch}.catJD = catJD;
                    lcData{Iwd,Ibatch} = transitSearch.handleNaNValues(lcData{Iwd,Ibatch},mms, mms.Nepoch);
...
%                    lcData{Iwd,Ibatch} = transitSearch.handleNaNValues(lcData, numel(catJD));
                   ...
                   results{Iwd,Ibatch}  = transitSearch.detectTransits(lcData{Iwd,Ibatch}, args);
            ...
            %finalTable = transitSearch.createTable2(Results, table, lcData, args)
                    i = 1;
                    Stats = transitSearch.countCells(lcData,table)
    ...
    %    end
        ...
        function lcData = extractLightCurve(mms, ra, dec,args)
            ...
            if ~isempty(ind)
                ...
                lcData.Ctrl = transitSearch.getCloseControl(mms, ind,args,ra,dec);
        ...
        function lc = getControl(mms, ind,args)
            ...
            if ~isempty(ctrlGroup)
                ...
                ctrlGroup =  ctrlGroup & NoNan > args.Ndet;
            ...
            if ~isempty(MarkedEvents)
                    ...
                    % Plot and save event ?
                      figure();
                         ...
                         set(gca,'YDir','reverse')
                         legend('WD','Events','LimMag','NaNs','Control Star')
                          title(sprintf('Detect in 2 consecutive points (2.5\sigma ) %s , %s',LC.Tel,LC.Date))
                          xlabel(sprintf('Coord : %.3f , %.3f ; Gmag = %.2f',lcData.Table.RA,...
        ...
        function detection = runMeanFilter(lcData, args)
            ...
            FlagRunMean = any(ResFilt.FlagCand, 1);
                      ...
                      if FlagRunMean
                            ...
                            detection = struct('events',find((ResFilt.FlagCand) == 1),'NanDet',NanDet, 'lightCurve', lcData.lc);
        ...
        function detection = detectAreaEvents(lcData, args)
            ...
            if any(D)
                    ...
                    % Plot and save event ?
                         ...
                         set(gca,'YDir','reverse')
                         legend('WD','Events','LimMag','NaNs','Control Star')
                         title(sprintf('Detect in Area Detecion %s , %s',LC.Tel{1},LC.Date))
                          xlabel(sprintf('Coord : %.3f , %.3f ; Gmag = %.2f',lcData.Table.RA,...
                              lcData.Table.Dec,lcData.Table.Gmag))
