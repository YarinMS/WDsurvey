%% 
function [MetaTable,mmm,ML] = MSHunter2(ML,mmm,MetaTable,mount, telescope, year, month, day, batchSize,args)
 ...
 arguments
   ...
   args.Date = sprintf('%i - %i - %i',year,month,day);
    ...
    for Ifield = 1: Nfields
        ...
        if ~strcmp(uniqueFields{Ifield}, 'MasterOT')
        ...
        for b = 1 : length(Batches)
            ...
            for cropID = 1:24
                ...
                if ~isempty(List.FileName)
                    ...
                    Ind = ms.coneSearch(60.2176,34.0772)
                    if isempty(Ind.Ind)
                        continue;
                    end
                    ms.bestMag;
                    ...
                    Ind = MMS.coneSearch(60.2176,34.0772)
                    if isempty(Ind.Ind)
                        continue;
                    end
                    mmm = [mmm ; {MS}]  ;
